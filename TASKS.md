...
### Wave 58 — Post-scrutiny hardening (4 fixes from /scrutinize)
- **Merged:** 2026-05-28 · `7057646` (PR #312) · **Issues:** #308, #309, #310, #311
- **Result:** 58a — audit log filter nav now persists when `?action=` active (second unfiltered query drives nav; guard changed to `distinct.length > 0`). 58b — `?action=` filter validated against `KNOWN_ACTIONS` allowlist in `page.tsx`; unknown values coerced to `null`. 58c — customers page `orders` query capped at `.limit(2000)` with visible truncation note; prevents silent PostgREST 1 000-row truncation. 58d — new `tests/db/send-later-no-address.test.ts` (3 pglite cases) pins the pre-DD-76 `coalesce('TBD')` behaviour so DD-76's required-address guard has a regression anchor. Suite 616 → 619. tsc clean, lint 0 problems.
