...
- **Wave 41 — Pre-Supabase hardening sweep** *(planning, 2026-05-24)*: 12-batch arc from a `/debug-mantra` audit. Live UX fixes (41a–41f: discount cap, mock-admin honesty, splits negative guard, proxy.ts verification, orphan-user ADR, /apply rate-limit + de-oracle) + latent RPC guards (41g–41j: create_order mixed-payments validation + discount cap + dead-CASE cleanup, registration-token de-oracle + length floor) + regression-test scaffold (41k) + ADR (41l). Full per-batch detail in [TASKS.md § Wave 41](../TASKS.md). Lands **before** DD-65 so Supabase wire-up doesn't ride latent bugs.
