...
- **Wave 43** (this PR) — events + `event_inventory` foundation (organic infra the DD plan assumed). `/app/events` (configured): create an event (status `planned`), **allocate active products** into `event_inventory` at their `default_starting_qty` (idempotent upsert), and start/close it. `lib/events/parse.ts` (+9 tests), `lib/auth/workspace.ts canManageEvents`, Server Actions `createEvent`/`allocateActiveProducts`/`setEventStatus` (workspace-scoped, role-gated), `EventsManagerLive`; demo `EventSetupClient` stays unconfigured. This gives `create_order` a running event + lockable stock. Suite 460 → 469.

Next: POS wired to the real catalog + active-event inventory (DD-55–64) → **DD-65 `create_order`** → DD-66. (Auth-gated live UIs reuse shipped components; reviewable on Vercel.)
